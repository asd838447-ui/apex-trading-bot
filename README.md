...
git clone https://github.com/asd838447-ui/apex-trading-bot.git
cd apex-trading-bot
...
Классификатор рыночных режимов (**Skill 06**) использует Скрытую Марковскую Модель (GaussianHMM). Перед запуском бота на реальных данных рекомендуется откалибровать модель на свежей рыночной истории:

```bash
# Обучение модели на истории BTCUSDT и сохранение весов в hmm_model.pkl
python scripts/train_hmm.py
```

*Скрипт автоматически загрузит исторические свечи через публичный API Binance Futures, обучит модель с 3 скрытыми состояниями (Flat, Trend, Volatile), откалибрует их и запишет веса в `server/skills/hmm_model.pkl`. Бот автоматически подгрузит эти параметры при следующем старте.*

## 🔌 Interactive API Documentation (Swagger)

FastAPI автоматически генерирует интерактивную OpenAPI-документацию ко всем эндпоинтам. 
Вы можете протестировать API, изучить схемы Pydantic моделей и отправить тестовые запросы:

- 📑 **Swagger UI:** `https://apex-trading-bot-xblr.onrender.com/docs`
- 📑 **ReDoc UI:** `https://apex-trading-bot-xblr.onrender.com/redoc`

## ⚠️ Render Free Tier Limitations

При развертывании бота на бесплатном тарифе Render Free Tier важно учитывать следующие технические особенности:

1. 😴 **15-минутный сон (Cold Starts):** При отсутствии запросов веб-сервис Render засыпает через 15 минут. Первый запрос после сна может выполняться **30-60 секунд**.
2. 🔄 **Разрыв WebSocket:** Во время сна WebSocket-соединение с биржей разрывается. Для торгового бота это критично! Рекомендуется настроить uptime-мониторинг (например, [UptimeRobot](https://uptimerobot.com) с периодичностью опроса 5-10 минут) для постоянного поддержания активности бота.
3. ⏳ **90 дней жизни БД:** Базы данных PostgreSQL/Redis на бесплатном тарифе автоматически удаляются Render через **90 дней** после создания. Регулярно делайте бэкапы/экспорт торговых журналов. Для постоянной работы рекомендуется переключить БД на Supabase Free Tier (без автоматического удаления).
